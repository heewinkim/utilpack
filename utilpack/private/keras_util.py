...
class SnapsKerasUtil(object):
    ...
    def getGenerators(root_path, input_resolution=(224, 224), batch_size=64,rescale=1./255,class_mode='categorical'):
        ...
        for path in [train_dir,validation_dir,test_dir]:
            if os.path.exists(path):
                generators.append(SnapsKerasUtil.getGenerator_fromDirectory(path,input_resolution,batch_size,rescale,class_mode))
            else:
    ...
    def getGenerator_fromDirectory(path, input_resolution=(224, 224), batch_size=64,rescale=1./255,class_mode='categorical'):
        ...
        if os.path.exists(path):
            ...
            raise SnapsError(ERROR_TYPES.RUNTIME_ERROR,'{} is not exist'.format(path))
    ...
    def evaluate_model(test_generator, model,print_=True,use_remote=False,**kwargs):
        ...
        try:
            ...
            raise SnapsERROR(ERROR_TYPES.RUNTIME_ERROR,'Need to compile model')
    ...
    def example_1():
        ...
        # 학습 데이터 파싱
        train_gen, validation_gen, test_gen = SnapsKerasUtil.getGenerators(args.data_dir, input_shape[:2], batch_size)
        train_step,validation_step,test_step = [SnapsKerasUtil.get_generatorStep(gen) for gen in [train_gen,validation_gen,test_gen]]
    
        # 데이터 이미지 출력
        SnapsKerasUtil.show_generator(train_gen)
        ...
        # 콜백함수 정의
        callbacks = SnapsKerasUtil.get_callback()
        ...
        # 정확도 확인
        SnapsKerasUtil.evaluate_model(test_gen,model)
    
    
        # 각 이미지별 예측 결과 상세 확인
        SnapsKerasUtil.test_model(test_gen, model, cols=8, steps=1, figsize=(20, 20))
    
    
        # CAM 확인
        SnapsKerasUtil.show_cam(test_gen,model,show_layer_idx=-5,figsize=(20,20),max_idx=16)
    ...
    def example_2():
        ...
        train_gen, validation_gen = SnapsKerasUtil.getGenerator_fromDataframe(args.annotation_path, batch_size, input_shape)
        train_step, validation_step = [SnapsKerasUtil.get_generatorStep(gen) for gen in [train_gen, validation_gen]]
    
        # SnapsKerasUtil.show_generator(train_gen,rows=2,cols=10,figsize=(25,25))
        ...
        # Get callbacks
        callbacks = SnapsKerasUtil.get_callback(use_ckpt=True, use_early_stop=True, early_stop_patience=10,
...
if __name__ == '__main__':
    ...
    path_list = SnapsImageUtil.get_pathlist('/Users/hian/Desktop/Data/image_data/snaps_image/thum')[:12]
    img_list = [cv2.imread(path) for path in path_list]

    gen = SnapsKerasUtil.getGenerator_fromData(img_list,[1 for _ in range(len(img_list))],resize=(224,224),batch_size=8)
